...
class MomentumSGD(Optimizer):
    ...
    def step(self):
        for p, dp in self.params:
            ...
            dp += self.reg * p
            delta = self.learn_rate * dp
            if p not in self.velocity:
                self.velocity[p] = - delta
            else:
                self.velocity[p] = self.momentum * self.velocity[p] - delta
            p += self.velocity[p]
...
class RMSProp(Optimizer):
    ...
    def step(self):
        for p, dp in self.params:
            ...
            dp += self.reg * p
            if p not in self.squared_avg:
                self.squared_avg[p] = (1 - self.decay) * torch.mul(dp, dp)
            else:
                self.squared_avg[p] = self.decay * self.squared_avg[p] + (1 - self.decay) * torch.mul(dp, dp)
            p -= (self.learn_rate/torch.sqrt(self.squared_avg[p].add(self.eps))) * dp
